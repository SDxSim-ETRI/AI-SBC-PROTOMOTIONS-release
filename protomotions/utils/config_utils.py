...
def parse_cli_overrides(override_strings: list) -> Dict[str, Any]:
    ...
    # Flatten: "key1=v1 key2=v2" passed as one shell-quoted string → split into tokens.
    # Split on whitespace that is immediately followed by a new "identifier.path=" pattern.
    expanded = []
    for s in override_strings:
        tokens = re.split(r"\s+(?=[A-Za-z_][A-Za-z0-9_.]*=)", s)
        expanded.extend(t.strip() for t in tokens if t.strip())
    override_strings = expanded
...
def backfill_dataclass_defaults(obj, _seen=None, _path="config"):
    """[ETRI 2026-08-25] 오래된 피클 config 에 이후 추가된 dataclass 필드를 기본값으로 채운다.

    왜 필요한가
    -----------
    `resolved_configs*.pt` 는 학습 시점의 config **인스턴스**를 피클한 것이다.
    나중에 코드에 dataclass 필드가 추가되면, 그 필드는 클래스에는 있지만
    **피클된 인스턴스의 __dict__ 에는 없다**. 그래서 새 코드가 접근하는 순간
    `AttributeError` 로 죽는다.

    실제 사례 (2026-08-25):
        AttributeError: 'IsaacLabSimulatorConfig' object has no attribute 'projectile'
        (base_simulator/simulator.py::_resolve_proj_config)
    → `SimulatorConfig.projectile` 은 그 체크포인트 이후에 추가된 필드다.

    구 체크포인트를 teacher/warm-start 로 재사용하는 작업(지형 전문가 증류 등)에서
    반복적으로 부딪히므로 범용 보정을 둔다. **기존 값은 절대 덮어쓰지 않는다** —
    없는 필드만 기본값으로 채운다.
    ...
    for f in dataclasses.fields(obj):
        if not hasattr(obj, f.name):
            if f.default is not dataclasses.MISSING:
                ...
                filled.append(f"{_path}.{f.name}")
            elif f.default_factory is not dataclasses.MISSING:  # type: ignore[misc]
                object.__setattr__(obj, f.name, f.default_factory())  # type: ignore[misc]
                filled.append(f"{_path}.{f.name}")
            # 기본값이 없는 필수 필드는 조용히 채울 수 없다 — 그대로 두어 원래 오류가 드러나게 한다.
            continue

        val = getattr(obj, f.name)
        if dataclasses.is_dataclass(val) and not isinstance(val, type):
            _, sub = backfill_dataclass_defaults(val, _seen, f"{_path}.{f.name}")
            filled.extend(sub)
        elif isinstance(val, (list, tuple)):
            for i, item in enumerate(val):
                if dataclasses.is_dataclass(item) and not isinstance(item, type):
                    _, sub = backfill_dataclass_defaults(item, _seen, f"{_path}.{f.name}[{i}]")
                    filled.extend(sub)
        elif isinstance(val, dict):
            for k, item in val.items():
                if dataclasses.is_dataclass(item) and not isinstance(item, type):
                    _, sub = backfill_dataclass_defaults(item, _seen, f"{_path}.{f.name}[{k!r}]")
                    filled.extend(sub)
    return obj, filled
