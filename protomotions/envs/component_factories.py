...
def contact_match_rew_factory(
    ...
    # [ETRI patch] contact_bodies 순서대로 바디별 가중치를 준다(예: toes 를 calcaneus
    # 보다 강하게 벌점). 미지정 시 기존과 동일한 균등 가중.
    # 소비하는 쪽(envs/rewards/regularization.py)은 이미 body_weights 를 지원한다.
    body_weights: list = None,
) -> MdpComponent:
    ...
    return MdpComponent(
        ...
        static_params=_contact_match_static_params(weight, zero_during_grace_period, body_weights),
    )
...
# [ETRI patch] 아래는 ETRI 추가분이다. 본문은 etri_component_factories.py 에 분리해
# upstream 수정을 최소화했다 (CLAUDE.md §3).
def _contact_match_static_params(weight, zero_during_grace_period, body_weights):
    params = {"weight": weight, "zero_during_grace_period": zero_during_grace_period}
    if body_weights is not None:
        import torch

        params["body_weights"] = torch.tensor(body_weights, dtype=torch.float32)
    return params


from protomotions.envs.etri_component_factories import (  # noqa: E402,F401
