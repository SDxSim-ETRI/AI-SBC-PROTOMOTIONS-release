...
def compute_contact_match_rew(
    ...
    Args:
        ...
        body_weights: Optional per-body multiplier [num_contact_bodies], in the same
            order as contact_body_ids. Lets specific bodies (e.g. toes) contribute
            more to the mismatch penalty than others (e.g. calcaneus). Defaults to
            uniform weighting (1.0 per body) when not provided.
    ...
    diff = torch.abs(sim_contacts_subset.float() - ref_contacts_subset.float())
    if body_weights is not None:
        diff = diff * body_weights.to(diff.device, diff.dtype)
