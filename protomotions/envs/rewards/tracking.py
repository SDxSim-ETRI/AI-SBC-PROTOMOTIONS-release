...
def compute_dof_pos_rew(
    ...
    dof_weights: Optional[Tensor] = None,
) -> Tensor:
    """Per-DOF joint angle tracking reward (exponential MSE).

    Complements body-position/rotation tracking (gt_rew/gr_rew) with a direct
    joint-angle term. Body-position error dilutes across all bodies and is
    largely insensitive to angle error at short-lever joints (e.g. elbow) --
    a rotational error there barely moves the hand in space, so gt_rew/gr_rew
    alone can tolerate joint angles that visibly diverge from the reference
    (e.g. elbow flexion locked near its limit, excess trunk flexion) while
    still reporting good body-position tracking. This term penalizes the
    joint-angle error directly and undiluted, one scalar per DOF.

    Uniform per-DOF weighting (dof_weights=None) plateaued in practice with
    one DOF (elbow_flex) stuck ~30deg off the reference while all other 22
    DOFs converged tightly -- a uniform mean gives it only 1/23 of the
    gradient, diluted by 22 already-converged DOFs pulling toward zero
    marginal loss (2026-07-21 skeleton walk retrain). dof_weights lets a
    specific lagging DOF get a larger share without a global coefficient/
    weight change that would perturb the DOFs that already converged.
    ...
    Args:
        ...
        coefficient: Exponential coefficient for error.
        dof_weights: Optional per-DOF multiplier [num_dofs], normalized to
            sum to num_dofs internally so the overall error scale stays
            comparable to the unweighted (uniform) case. None = uniform.
    ...
    num_dofs = current_dof_pos.shape[-1]
    w = dof_weights.to(current_dof_pos.device, current_dof_pos.dtype)
    w = w * (num_dofs / w.sum())
    weighted_sq_err = w * (current_dof_pos - ref_dof_pos).pow(2)
    return torch.exp(coefficient * weighted_sq_err.mean(dim=-1))
