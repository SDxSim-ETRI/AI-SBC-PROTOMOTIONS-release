...
class BaseEnv:
    ...
    def _process_action(self, action: Tensor, context: EnvContext) -> Dict[str, Tensor]:
        ...
        # [ETRI 2026-08-25] 미참조 DOF 의 PD 목표 고정.
        # 근거·측정치는 EnvConfig.etri_zero_dof_targets docstring 참조.
        _zero = getattr(self.config, "etri_zero_dof_targets", None)
        if _zero:
            pa = result.get("processed_action", None)
            if pa is not None:
                if getattr(self, "_etri_zero_idx", None) is None:
                    self._etri_zero_idx = torch.as_tensor(
                        list(_zero), dtype=torch.long, device=pa.device
                    )
                    print(
                        f"[ETRI] 미참조 DOF {len(_zero)}개의 PD 목표를 0 rad 로 고정: {list(_zero)}"
                    )
                pa = pa.clone()
                pa[..., self._etri_zero_idx] = 0.0
                result["processed_action"] = pa
        return result
