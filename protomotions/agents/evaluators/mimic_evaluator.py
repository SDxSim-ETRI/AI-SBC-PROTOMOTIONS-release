...
class MimicEvaluator(BaseEvaluator):
    ...
    def _update_motion_sampling_weights(self) -> None:
        ...
        # Prevent float32 overflow: failure_discount = base^eval_metrics_every can be
        # near-zero when eval_metrics_every is large, causing inf when dividing.
        # Replace inf/nan then normalize so relative proportions are preserved.
        new_weights = torch.nan_to_num(new_weights, nan=1.0, posinf=1e30, neginf=0.0)
        weight_sum = new_weights.sum()
        if weight_sum > 0:
            new_weights = new_weights / weight_sum * len(new_weights)
        # upstream(#2026-08): floor the weights *last* so the success discount cannot
        # compound a motion down to ~0 (sampling collapse). Applied after the
        # normalization above so the floor is what actually holds.
