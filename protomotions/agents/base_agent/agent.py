...
class BaseAgent:
    ...
    def _load_model_state_dict(self, model_state_dict):
        ...
        # [ETRI patch] obs/action 차원이 바뀐 체크포인트로 warm-start 할 때
        # 엄격한 로드는 RuntimeError 로 죽는다. 형태가 맞는 텐서만 싣는 폴백.
        # 본문은 protomotions/agents/etri_load_utils.py 에 있다 (CLAUDE.md §2).
        try:
            self.model.load_state_dict(model_state_dict)
        except RuntimeError as e:
            from protomotions.agents.etri_load_utils import partial_load_model_state

            log.warning(f"[ETRI] 엄격한 로드 실패({e}) — 부분 로드로 폴백")
            partial_load_model_state(self.model, model_state_dict)
