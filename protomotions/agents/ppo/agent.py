...
class PPO(BaseAgent):
    ...
    def _load_ppo_training_state(self, state_dict, require_optimizers: bool):
        # [ETRI patch] upstream 은 optimizer state 가 **없을 때**를 관용한다.
        # 모델 구조가 바뀌면 state 는 있지만 param_group 형태가 달라 로드가 깨지므로
        # 호환성까지 검사한다. 본문은 protomotions/agents/etri_load_utils.py.
        # 둘 다 실리지 않았으면 adaptive LR 복원도 건너뛴다 (optimizer 없이 LR 만
        # 스케일하면 어긋난다).
        from protomotions.agents.etri_load_utils import (
            load_optimizer_state_if_compatible,
        )

        _actor_ok = load_optimizer_state_if_compatible(
            self.actor_optimizer, state_dict, "actor_optimizer"
        )
        _critic_ok = load_optimizer_state_if_compatible(
            self.critic_optimizer, state_dict, "critic_optimizer"
        )
        if require_optimizers and not (_actor_ok and _critic_ok):
            log.warning(
                "[ETRI] optimizer state 를 싣지 못했습니다 — 초기 상태로 학습을 이어갑니다."
            )
