...
class NewtonSimulator(Simulator):
    ...
    def __init__(
        ...
        # [ETRI patch] 카메라 방위각 키([ ] B N)를 upstream 등록 API 로 추가.
        # 본문은 protomotions/simulator/newton/etri_viewer_keys.py (CLAUDE.md §2).
        from protomotions.simulator.newton.etri_viewer_keys import (
    ...
    def _create_simulation(self) -> None:
        ...
        # Persistent external-wrench buffer — MUST be allocated before CUDA graph
        # capture below: the captured copy kernel in _push_external_body_wrench()
        # bakes in this buffer's pointer, and set_external_body_wrench() mutates
        # its contents in place at runtime.
        self._external_wrench_sim_buf = torch.zeros(
            self.num_envs,
            1,
        ...
        self._external_wrench_wp = wp.from_torch(
            self._external_wrench_sim_buf, dtype=wp.spatial_vector, requires_grad=False
        )
    ...
    def set_external_body_wrench(self, wrench: Optional[torch.Tensor]) -> None:
        """Newton override: write into the persistent wrench buffer (in-place).

        _simulate() is captured in a CUDA graph at init time, so the per-substep
        copy kernel inside it is baked with fixed array pointers. Runtime updates
        therefore must mutate the SAME buffer the captured kernel reads from —
        allocating a fresh tensor per step (the base-class pattern) would be
        silently ignored during graph replay.
        """
        if self._external_wrench_sim_buf is None:
            # Buffer allocation failed/skipped at init — fall back to base behavior
            # (works only in eager mode).
            super().set_external_body_wrench(wrench)
            return
        if wrench is None:
            self._external_wrench_sim_buf.zero_()
        else:
            self._external_wrench_sim_buf[:, 0] = wrench[
                :, self.data_conversion.body_convert_to_sim, :
            ]

    def _push_external_body_wrench(self) -> None:
        """Copy the persistent wrench buffer into state_0.body_f (every substep).

        Newton clears body_f every substep (state_0.clear_forces()), so this runs
        every substep — see call site in _simulate(). Runs unconditionally (buffer
        defaults to zeros) so the copy kernel is captured into the CUDA graph.
        """
        if self._external_wrench_wp is None:
            return
        self.robot_view.set_attribute("body_f", self.state_0, self._external_wrench_wp)
    ...
    def _write_viewport_to_file(self, file_name: str) -> None:
        # [ETRI patch] 이 클래스에는 같은 이름의 정의가 위(PIL 판)에도 있으나 파이썬은
        # 뒤에 정의된 이 메서드를 쓴다. headless(`--headless`) 로 녹화하면 viewer 가
        # None 이라 AttributeError 로 죽던 것을 가드로 막는다 — 위쪽 정의에는 이미
        # 같은 가드가 있었지만 가려져 동작하지 않았다.
        if self.viewer is None or not hasattr(self.viewer, "get_frame"):
            return
