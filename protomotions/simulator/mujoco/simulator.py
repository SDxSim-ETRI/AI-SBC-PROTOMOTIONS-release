...
class MujocoSimulator(Simulator):
    ...
    def __init__(
        ...
        # Headless recording: frames are saved via the offscreen mujoco.Renderer
        # (record.py:_write_viewport_to_file). In headless mode no glfw viewer is
        # opened, so the offscreen GL context (set MUJOCO_GL=egl) works alone —
        # avoiding the viewer/offscreen context clash that crashes non-headless
        # recording. Enable the record path even though self.headless is True.
        if self.headless:
    ...
    def _load_mjcf_stripped(
        ...
        terrain=None,
    ) -> Tuple[mujoco.MjModel, Optional[np.ndarray]]:
        ...
        4. Add heightfield terrain geom (if terrain is non-flat)

        Returns (model, hfield_data) where hfield_data is normalized float32 [0,1]
        array to be assigned to model.hfield_data after loading, or None for flat terrain.
        """
        ...
        # 2. Add scene setup (visual, textures, lighting)
        MujocoSimulator._add_visual_settings(root)
        MujocoSimulator._add_scene_assets(root)

        # 3. Add terrain heightfield (before ground, so we can skip ground if terrain present)
        hfield_data = MujocoSimulator._add_terrain_hfield(root, terrain)
        MujocoSimulator._add_ground_and_light(root, skip_ground=hfield_data is not None)

        # 4. Add projectile bodies
        ...
        return model, hfield_data

    @staticmethod
    def _add_terrain_hfield(
        root: ET.Element, terrain
    ) -> Optional[np.ndarray]:
        """Add MuJoCo heightfield asset+geom to MJCF for non-flat terrain.

        Returns normalized float32 [0,1] hfield data (row-major, MuJoCo convention)
        to be assigned to model.hfield_data after model creation, or None if the
        terrain is flat or absent.

        Axis convention:
          ProtoMotions: height_field_raw[row, col], row=X axis, col=Y axis.
          MuJoCo hfield: stored as [mj_row, mj_col], mj_row=Y axis, mj_col=X axis.
          → transpose required.
        """
        if terrain is None or not hasattr(terrain, "height_field_raw"):
            return None

        hfr = terrain.height_field_raw  # int16 [tot_rows, tot_cols]
        if int(hfr.max()) <= 0:
            return None  # flat terrain

        hscale = terrain.horizontal_scale  # m/pixel
        vscale = terrain.vertical_scale    # m/height_unit

        # Convert to float metres; transpose for MuJoCo axis convention
        elevation = hfr.astype(np.float32) * vscale  # [tot_rows, tot_cols]
        elevation = elevation.T                        # [Y-dim, X-dim]

        nrow, ncol = elevation.shape   # MuJoCo nrow=Y-dim, ncol=X-dim
        x_half = ncol * hscale / 2.0
        y_half = nrow * hscale / 2.0
        max_elev = float(elevation.max())
        if max_elev < 1e-6:
            return None

        # Add <hfield> asset — data will be set after model creation
        asset = root.find("asset")
        if asset is None:
            asset = ET.SubElement(root, "asset")
        hfield_elem = ET.SubElement(asset, "hfield")
        hfield_elem.set("name", "terrain")
        hfield_elem.set("nrow", str(nrow))
        hfield_elem.set("ncol", str(ncol))
        # size: x_half_extent, y_half_extent, z_scale (top), z_base (bottom depth)
        hfield_elem.set("size", f"{x_half:.4f} {y_half:.4f} {max_elev:.4f} 0.1")

        # Remove any existing floor/plane geoms (robot MJCF may include one)
        worldbody = root.find("worldbody")
        for geom in worldbody.findall("geom"):
            name = geom.get("name", "").lower()
            gtype = geom.get("type", "").lower()
            if "floor" in name or "ground" in name or gtype == "plane":
                worldbody.remove(geom)
                log.info("Removed existing floor/plane geom '%s' (replaced by terrain)", geom.get("name", ""))

        # Add terrain geom: center at (x_half, y_half) so terrain spans [0,2*x_half]×[0,2*y_half]
        ...
        terrain_geom.set("pos", f"{x_half:.4f} {y_half:.4f} 0")
        terrain_geom.set("friction", "1.5 0.02 0.001")
        terrain_geom.set("rgba", "0.6 0.6 0.6 1")

        log.info(
            "Terrain heightfield: %d×%d grid, world span %.1f×%.1f m, "
            "height range [0, %.3f] m",
            nrow, ncol, 2 * x_half, 2 * y_half, max_elev,
        )

        return (elevation / max_elev).astype(np.float32)
    ...
    @staticmethod
    def _add_ground_and_light(
        root: ET.Element, skip_ground: bool = False
    ) -> None:
        ...
        if not skip_ground:
            ...
            if not has_ground:
                ...
                # High friction to approximate Newton's MAX-combine mode.
                # Newton uses max(μ_body, μ_floor); MuJoCo uses min by default,
                # so raise the floor friction so the effective value stays ~1.0.
                ground.set("friction", "1.5 0.02 0.001")
                print("  Added ground plane to MJCF (friction=1.5)")
    ...
    def _create_simulation(self) -> None:
        ...
        self.model, hfield_data = self._load_mjcf_stripped(
            asset_path, self._proj_config, terrain=self.terrain
        )
        self.data = mujoco.MjData(self.model)
        self._set_robot_geom_friction()

        # Inject heightfield elevation data (normalized [0,1]) into the loaded model
        if hfield_data is not None:
            assert self.model.nhfield > 0, "hfield_data returned but model has no hfields"
            self.model.hfield_data[:] = hfield_data.flatten()
            log.info("Terrain hfield data injected (%d values)", len(hfield_data.flatten()))
    ...
    def _build_actuator_mapping(self) -> None:
        ...
        # Map: for each joint-driven actuator, find which DOF it controls
        # Skip muscle/tendon actuators (mjTRN_TENDON) — they don't map to a single DOF
        actuator_to_dof = {}
        for act_idx in range(self.model.nu):
            trntype = self.model.actuator_trntype[act_idx]
    ...
    def _init_viewer(self) -> None:
        ...
        # Disable actuator/force/joint axis arrows by default; keep tendon visible
        self.viewer.opt.flags[mujoco.mjtVisFlag.mjVIS_ACTUATOR] = False
    ...
    def _ensure_offscreen_gl_context(self) -> None:
        """Repair mujoco's offscreen GL context resolution (once).

        pose_lib imports dm_control with ``MUJOCO_GL=disabled`` (+
        ``PYOPENGL_PLATFORM=glx``) before restoring the env var. That first
        import of ``mujoco.rendering.classic.gl_context`` caches
        ``GLContext = <undefined>``, so a later ``mujoco.Renderer`` crashes with
        ``AttributeError: ... has no attribute 'GLContext'`` even though
        MUJOCO_GL is now valid. Reload the module with a real offscreen backend
        (EGL, headless-friendly; needs PYOPENGL_PLATFORM unset/egl).
        """
        if getattr(self, "_gl_context_fixed", False):
            return
        import importlib
        import mujoco.rendering.classic.gl_context as _glc

        if getattr(_glc, "GLContext", None) is None:
            backend = (os.environ.get("MUJOCO_GL") or "").lower().strip()
            if backend in ("", "disable", "disabled", "off", "false", "0"):
                backend = "egl"
            os.environ["MUJOCO_GL"] = backend
            # EGL/OSMesa refuse to load if PYOPENGL_PLATFORM=glx (pose_lib set it).
            if backend in ("egl", "osmesa"):
                if os.environ.get("PYOPENGL_PLATFORM", "").lower() not in ("", backend):
                    os.environ.pop("PYOPENGL_PLATFORM", None)
            importlib.reload(_glc)
        self._gl_context_fixed = True

    def _write_viewport_to_file(self, file_name: str) -> None:
        """Render current view to an offscreen file (headless recording)."""
        self._ensure_offscreen_gl_context()

        # Cache the renderer + a tracking camera (the robot's root moves far from
        # the origin, so the default free camera would not frame it).
        if getattr(self, "_offscreen_renderer", None) is None:
            self._offscreen_renderer = mujoco.Renderer(
                self.model, height=480, width=640
            )
            cam = mujoco.MjvCamera()
            cam.type = mujoco.mjtCamera.mjCAMERA_TRACKING
            cam.trackbodyid = 1  # pelvis (first non-world body)
            cam.distance = 3.0
            cam.elevation = -10.0
            cam.azimuth = 180.0
            self._offscreen_camera = cam

        renderer = self._offscreen_renderer
        renderer.update_scene(self.data, camera=self._offscreen_camera)
    ...
    def set_window_title(self, title: str) -> None:
        """Display title text as a viewport overlay."""
        if not self.headless and self.viewer is not None and self._viewer_initialized:
            self.viewer.set_texts(((None, None, title, None),))
