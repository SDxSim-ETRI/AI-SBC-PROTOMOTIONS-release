...
class Simulator(RecordingMixin, ABC):
    ...
    def __init__(
        ...
        # Optional per-body external wrench (force+torque), e.g. for cable/tendon
        # forces that engine-native tendon actuators can't provide (Newton/IsaacLab
        # have no tendon actuator support). [num_envs, num_bodies, 6] world-frame
        # wrench applied at each body's COM. None when unused (default, no-op).
        self._external_body_wrench: Optional[torch.Tensor] = None
    ...
    def set_external_body_wrench(self, wrench: Optional[torch.Tensor]) -> None:
        """
        Set a per-body external wrench (force+torque) to be applied every substep
        until overwritten or cleared.

        This exists for control mechanisms neither Newton nor IsaacLab support
        natively (e.g. a tendon actuator applying force directly along a cable
        path) — computing the force in Cartesian/body space and pushing it as a
        generic wrench works identically on any backend that implements
        `_push_external_body_wrench()`.

        Args:
            wrench: [num_envs, num_bodies, 6] world-frame wrench (force xyz,
                torque xyz) applied at each body's center of mass, in common
                (ProtoMotions) body ordering. Pass None to stop applying.
        """
        self._external_body_wrench = wrench

    def _push_external_body_wrench(self) -> None:
        """
        Flush `self._external_body_wrench` into the simulator's native force
        buffer. Called every substep by backends that support it.

        Default no-op: backends that don't implement this (or tasks that never
        call `set_external_body_wrench()`) are unaffected.
        """
        pass
