...
def _soft_finite_check(name: str, tensor) -> None:
    """Replace non-finite values with 0 and warn; affected envs auto-reset via tracking error."""
    if tensor is None or torch.all(torch.isfinite(tensor)):
        return
    bad_mask = ~torch.isfinite(tensor)
    tensor[bad_mask] = 0.0
    warnings.warn(f"[physics] {name} non-finite (zeroed, env will auto-reset)")
...
class RobotState(BaseBatchedState):
    ...
    def __post_init__(self):
        ...
        self._check_finite("dof_forces", self.dof_forces)

    def _check_finite(self, name: str, tensor) -> None:
        _soft_finite_check(name, tensor)
...
class RootOnlyState(BaseBatchedState):
    ...
    def __post_init__(self):
        _soft_finite_check("root_pos", self.root_pos)
        _soft_finite_check("root_rot", self.root_rot)
        _soft_finite_check("root_vel", self.root_vel)
        _soft_finite_check("root_ang_vel", self.root_ang_vel)
...
class ResetState(BaseBatchedState):
    ...
    def __post_init__(self):
        ...
        _soft_finite_check("root_vel", self.root_vel)
        _soft_finite_check("root_ang_vel", self.root_ang_vel)
        _soft_finite_check("dof_pos", self.dof_pos)
        _soft_finite_check("dof_vel", self.dof_vel)
