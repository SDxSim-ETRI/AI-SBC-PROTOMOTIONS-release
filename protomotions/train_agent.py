...
def create_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override output directory (default: results/<experiment-name>)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="If set, automatically copy the final checkpoint to this directory after training completes.",
    )
    parser.add_argument(
        "--use-wandb",
        action="store_true",
...
def main():
    ...
    # ===================================================================
    save_dir = Path(args.save_dir) if getattr(args, "save_dir", None) else Path("results") / args.experiment_name
    ...
    fabric_config = FabricConfig(
        accelerator="cpu" if getattr(args, "simulator", None) == "mujoco" else "gpu",
        devices=args.ngpu,
    ...
    # In warm_start mode, load_parameters() restores the source checkpoint's epoch
    # counter (e.g., Newton epoch 53,090), which would cause training to exit
    # immediately if max_epochs < source epoch. Reset counters so fine-tuning
    # starts from epoch 0 with fresh timing/score tracking.
    if mode == "warm_start":
    ...
    # ===================================================================
    # 8. Auto-promote checkpoint (if --checkpoint-dir is given)
    # ===================================================================
    if fabric.global_rank == 0 and getattr(args, "checkpoint_dir", None):
        import shutil

        dst = Path(args.checkpoint_dir)
        dst.mkdir(parents=True, exist_ok=True)
        src_ckpt = save_dir / "last.ckpt"
        if src_ckpt.exists():
            for fname in ["last.ckpt", "resolved_configs.pt", "resolved_configs_inference.pt", "config.yaml"]:
                src_f = save_dir / fname
                if src_f.exists():
                    shutil.copy2(src_f, dst / fname)
            epoch = agent.current_epoch
            date_str = __import__("datetime").datetime.now().strftime("%Y-%m-%d %H:%M")
            (dst / "INFO.md").write_text(
                f"# Checkpoint Info\n\n"
                f"| 항목 | 내용 |\n|------|------|\n"
                f"| 소스 | `{save_dir}` |\n"
                f"| 보관일 | {date_str} |\n"
            ...
            print(f"[checkpoint-dir] Saved to: {dst}  (epoch {epoch})")
