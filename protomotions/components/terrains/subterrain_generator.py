...
def discrete_obstacles_subterrain(
    ...
    """
    Generate a terrain with rectangular obstacles.

    Parameters:
        subterrain (terrain): the terrain
        max_height (float): maximum height of the obstacles [meters]
        ...
        platform_size (float): size of the flat platform at the center of the terrain [meters]
        bevel_size (float): width of the chamfered ramp at obstacle edges [meters]. 0 = sharp corners.
    Returns:
        terrain (SubTerrain): update terrain
    """
    # switch parameters to discrete units
    ...
    if bevel_size > 0 and max_height > 0:
        from scipy.ndimage import distance_transform_edt

        bevel_cells = bevel_size / subterrain.horizontal_scale
        obstacle_mask = subterrain.height_field_raw >= max_height
        dist = distance_transform_edt(~obstacle_mask)
        bevel_mask = (dist > 0) & (dist < bevel_cells)
        bevel_heights = np.round(
            max_height * (1.0 - dist[bevel_mask] / bevel_cells)
        ).astype(int)
        subterrain.height_field_raw[bevel_mask] = np.maximum(
            subterrain.height_field_raw[bevel_mask], bevel_heights
        )
