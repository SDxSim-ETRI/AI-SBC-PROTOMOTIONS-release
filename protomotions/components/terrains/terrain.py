...
class Terrain:
    ...
    def __init__(self, config: TerrainConfig, num_envs: int, device) -> None:
        ...
        self.num_scenes_per_column = (
            max(math.floor(length / self.spacing_between_scenes), 1)
            if self.spacing_between_scenes > 0
            else 1
        )
    ...
    def sample_flat_locations(self, num_envs):
        # [ETRI patch] flat terrain 이 하나도 없을 때(예: stairs 100% 구성) walkable
        # 전체에서 샘플링한다. 그대로 두면 randint(0, 0) 으로 죽는다.
        # upstream 의 _sample_coordinate_pairs 는 x/y 를 같은 인덱스로 짝지어 뽑는다
        # (독립적으로 뽑으면 평지가 아닌 (x, y) 조합이 나옴). walkable 도 같은 방식.
        # 이 분기는 is_valid_spawn_location assert 를 건너뛴다(walkable 은 평지가 아니므로).
        if self.flat_x_coords.shape[0] == 0:
            return self._sample_coordinate_pairs(
                self.walkable_x_coords, self.walkable_y_coords, num_envs
            )
    ...
    def _generate_subterrain(self, subterrain, terrain_type, difficulty):
        """terrain_type 정수(0~7)로 subterrain을 직접 생성한다.
        0=smooth_slope, 1=rough_slope, 2=stairs_up, 3=stairs_down,
        4=discrete, 5=stepping_stones, 6=poles, 7=flat
        """
        slope = difficulty * self.config.slope_scale
        _stairs_max = self.config.pyramid_stairs_step_height_max
        step_height = 0.05 + ((_stairs_max - 0.05) if _stairs_max is not None else 0.175) * difficulty
        discrete_h = (
            self.config.discrete_obstacles_min_height
            + difficulty * (
                self.config.discrete_obstacles_max_height
                - self.config.discrete_obstacles_min_height
            )
        )
        stepping_size = 2 - 1.8 * difficulty

        if terrain_type == 0:  # smooth_slope
            pyramid_sloped_subterrain(subterrain, slope=slope, platform_size=3.0)
        elif terrain_type == 1:  # rough_slope
            pyramid_sloped_subterrain(subterrain, slope=slope, platform_size=3.0)
            _amp = self.config.rough_terrain_amplitude
            random_uniform_subterrain(
                subterrain,
                min_height=-_amp, max_height=_amp,
                step=min(0.025, _amp),
                downsampled_scale=self.config.rough_terrain_period,
            )
        elif terrain_type == 2:  # stairs_up (center is highest)
            _step_h = abs(self.config.pyramid_stairs_step_height or step_height)
            pyramid_stairs_subterrain(
                subterrain, step_width=self.config.pyramid_stairs_step_width,
                step_height=_step_h, platform_size=self.config.pyramid_stairs_platform_size,
            )
        elif terrain_type == 3:  # stairs_down (center is lowest)
            _step_h = abs(self.config.pyramid_stairs_step_height or step_height)
            pyramid_stairs_subterrain(
                subterrain, step_width=self.config.pyramid_stairs_step_width,
                step_height=-_step_h, platform_size=self.config.pyramid_stairs_platform_size,
            )
        elif terrain_type == 4:  # discrete_obstacles
            discrete_obstacles_subterrain(
                subterrain, discrete_h, 1.0, 2.0, 40,
                platform_size=3.0, bevel_size=self.config.discrete_obstacles_bevel_size,
            )
        elif terrain_type == 5:  # stepping_stones
            stepping_stones_subterrain(
                subterrain, stone_size=stepping_size,
                stone_distance=0.1, max_height=0.0, platform_size=3.0,
            )
        elif terrain_type == 6:  # poles
            poles_subterrain(subterrain=subterrain, difficulty=difficulty)
        # terrain_type == 7: flat → subterrain은 그대로(zero height field)
    ...
    def curriculum(self, n_subterrains_per_level, n_levels):
        # terrain_sequence: 열마다 지형 타입을 직접 지정 (terrain_proportions 무시)
        # 인덱스: 0=smooth_slope, 1=rough_slope, 2=stairs_up, 3=stairs_down,
        #         4=discrete, 5=stepping_stones, 6=poles, 7=flat
        # 예시: [7, 0, 4, 0, 4, 0, 4, 0, 4, 7] → flat|slope|disc|...|flat
        _seq = self.config.terrain_sequence  # None이면 비활성
        ...
        for subterrain_idx in range(n_subterrains_per_level):
            for level_idx in range(n_levels):
                ...
                if _seq is not None:
                    terrain_type = _seq[subterrain_idx % len(_seq)]
                    self._generate_subterrain(subterrain, terrain_type, difficulty)
                    if terrain_type == 6:  # poles: walkable field 별도 처리
                        self.walkable_field_raw[start_x:end_x, start_y:end_y] = (
                            subterrain.height_field_raw != 0
                        )
                    if terrain_type == 7:  # flat: flat_field 처리
                    ...
                    if terrain_type == 3:  # stairs_down: 중앙 플랫폼(가장 낮은 지점)에 스폰
                        cx = (start_x + end_x) // 2
                        cy = (start_y + end_y) // 2
                        plat_px = int(self.config.pyramid_stairs_platform_size / 2 / self.horizontal_scale)
                        self.flat_field_raw[
                ...
                if choice < self.proportions[0]:
                    ...
                    _step_h = (
                        abs(self.config.pyramid_stairs_step_height)
                        if self.config.pyramid_stairs_step_height is not None
                        else step_height
                    )
                    _step_w = self.config.pyramid_stairs_step_width
                    _plat_sz = self.config.pyramid_stairs_platform_size
                    is_stairs_down = choice >= self.proportions[2]
                    if not is_stairs_down:
                        _step_h = -_step_h  # stairs up: center is lowest (normalized to 0 later)
                    pyramid_stairs_subterrain(
                        subterrain,
                        step_width=_step_w,
                        step_height=_step_h,
                        platform_size=_plat_sz,
                    )
                    if is_stairs_down:  # stairs_down: 중앙 플랫폼(가장 낮은 지점)에 스폰
                        cx = (start_x + end_x) // 2
                        cy = (start_y + end_y) // 2
                        plat_px = int(_plat_sz / 2 / self.horizontal_scale)
                        self.flat_field_raw[
                            cx - plat_px : cx + plat_px,
                            cy - plat_px : cy + plat_px,
                        ] = 0
                    ...
                    discrete_obstacles_subterrain(
                        ...
                        40,
                        platform_size=3.0,
                        bevel_size=self.config.discrete_obstacles_bevel_size,
                    )
