...
# dm_control is used only for MJCF parsing, not rendering.
# Override MUJOCO_GL unconditionally: dm_control must not initialize any
# PyOpenGL renderer here, or it will conflict with Newton's GLX-based viewer.
# PYOPENGL_PLATFORM=glx is set so PyOpenGL selects GLX when Newton's viewer
# imports it later (must be set before any OpenGL import).
_prev_mujoco_gl = os.environ.get("MUJOCO_GL")
os.environ["MUJOCO_GL"] = "disabled"
os.environ.setdefault("PYOPENGL_PLATFORM", "glx")
from dm_control import mjcf
# Restore original MUJOCO_GL so other dm_control usage is unaffected.
if _prev_mujoco_gl is None:
    os.environ.pop("MUJOCO_GL", None)
else:
    os.environ["MUJOCO_GL"] = _prev_mujoco_gl
import logging
import sys
    ...
    def _traverse(mjcf_body, parent_idx, inherited_default=None):
        ...
        if body_idx == 0:  # Root Body
            ...
            if len(joints) > 0:
                assert (
                    1 <= len(joints) <= 3
                ), f"Body {body_name} has {len(joints)} joints, expected 1, 2, or 3"
                for joint in joints:
