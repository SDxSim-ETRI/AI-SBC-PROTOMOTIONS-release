# [C5 ablation ⑤] 해부학적 범위 + 원본 PD + velocity_limit만 100 — velocity 단독 격리. 2026-08-05
...
@dataclass
class SkeletonTorque27DofAnatRangeVelOnlyRobotConfig(RobotConfig):
    """
    ...
    asset: RobotAssetConfig = field(
        default_factory=lambda: RobotAssetConfig(
            asset_root="tasks_for_skeleton/mimic_smpl_to_skeleton_motions48/data/assets/mjcf_skeleton_torque_newton_anatrange",
        ...
        default_factory=lambda: ControlConfig(
            ...
            override_control_info={
                "hip_(flexion|adduction)_[rl]": ControlInfo(
                    stiffness=200.0, damping=20.0, effort_limit=600, velocity_limit=100
                ),
                ...
                "ankle_angle_[rl]": ControlInfo(
                    stiffness=100.0, damping=10.0, effort_limit=500, velocity_limit=100
                ),
                "lumbar_(extension|bending|rotation)": ControlInfo(
                    stiffness=100.0, damping=10.0, effort_limit=160, velocity_limit=100
                ),
                "arm_(flex|add|rot)_[rl]": ControlInfo(
                    stiffness=100.0, damping=10.0, effort_limit=250, velocity_limit=100
                ),
                ...
                "wrist_(flex|dev)_[rl]": ControlInfo(
                    stiffness=100.0, damping=10.0, effort_limit=100, velocity_limit=100
                ),
            },
        )
    )
