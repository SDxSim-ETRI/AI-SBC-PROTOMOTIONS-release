...
def robot_config(robot_name: str, **updates) -> RobotConfig:
    ...
    """
    # [ETRI patch] ETRI 로봇 13개는 etri_registry.py 에 분리 등록 (CLAUDE.md §2).
    # 미등록 이름이면 None 이 돌아오고 아래 upstream 분기가 그대로 처리한다.
    # upstream 행을 수정하지 않고 삽입만 하므로 다음 merge 에서 충돌 위험이 낮다.
    from protomotions.robot_configs.etri_registry import etri_robot_config

    _etri_config = etri_robot_config(robot_name)
    if _etri_config is not None:
        if updates:
            _etri_config.update_fields(**updates)
        return _etri_config
