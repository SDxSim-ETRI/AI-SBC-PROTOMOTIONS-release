...
#
"""Active Cable (DOFC A버전) — 35 DOF suit (31 skeleton + 4 cables), Newton/IsaacLab.

27 DOF 버전(mimic_newton_active_cable)과 동일한 로직이나 DOF 인덱스가 다름:
  hip_flexion_l: DOF 5 → 7
  slide1/2/3/4:  DOF 23–26 → 31–34

Warm-start chain:
  v29_isaaclab_active_cable_rough27 (27 DOF) → zero-pad 27→35 DOF → 학습

학습 명령어 (flat, Newton):
    python protomotions/train_agent.py \\
        --robot-name etrisuit_active_cable \\
        --simulator newton \\
        --experiment-path examples/experiments/mimic_newton_active_cable_31dof/mlp.py \\
        --experiment-name 31dof/newton_suit_active_cable_walk_flat \\
        --motion-file data/motion_for_trackers/31dof/softsuit_walk.pt \\
        --num-envs 2048 \\
        --batch-size 8192 \\
        --checkpoint checkpoints/v29_isaaclab_active_cable_rough27/last_35dof.ckpt
"""
import argparse
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    return EnvConfig(
        # ActiveCableEnv31Dof: DOFC A버전 with 35 DOF indices (slide=31–34, hip_l=7)
        _target_="protomotions.envs.base_env.etrisuit_active_cable_env.ActiveCableEnv31Dof",
