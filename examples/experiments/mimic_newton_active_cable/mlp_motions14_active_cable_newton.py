...
#
"""
Active Cable motions14 Newton 재학습 설정 (failure_discount=0.3)
===============================================================

mlp.py 대비 변경점:
  - motion_weights_update_failure_discount: 0 → 0.3
      실패한 고난도 모션(highknee, onehopforward, lunge 등)이 가중치 0이 되어
      학습에서 영구 제외되는 문제 해결. 실패해도 일정 확률로 계속 샘플링되어
      점진적으로 개선됨.
  - motion_weights_update_success_discount: 0.999 (동일)
  - 그 외 ActiveCableEnv(_target_), reward, network, termination 설정 모두 동일
"""
import argparse
...
def agent_config(
    ...
    # 네트워크 구조는 mlp.py와 동일하게 유지 (가중치 warm_start 호환)
    ...
    agent_cfg: PPOAgentConfig = PPOAgentConfig(
        ...
        evaluator=MimicEvaluatorConfig(
            ...
            motion_weights_rules=MotionWeightsRulesConfig(
                ...
                # 0(기본) → 0.3: 실패 모션도 지속 샘플링되어 고난도 동작 개선
                motion_weights_update_failure_discount=0.3,
            ),
        ),
