...
#
"""
v23 Active Cable (DOFC A버전) — IsaacLab 파인튜닝
=================================================

v21(Newton active cable) warm_start → IsaacLab로 파인튜닝.
단일 모션(constspeed_reduced_humanoid) 사용.
네트워크 구조는 v21과 동일 (가중치 호환).

IsaacLab 파인튜닝 설정:
  - actor lr: 2e-6 (Newton 2e-5 대비 10배 낮춤)
  - critic lr: 1e-5 (Newton 1e-4 대비 10배 낮춤)
  - num_envs: 1024, batch_size: 4096
...
학습 명령어:
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/train_agent.py \\
        ...
        --experiment-name mimic_isaaclab_active_cable \\
        --motion-file data/motion_for_trackers/skeleton_torque_suit_walk_constspeed_reduced_humanoid.pt \\
        --checkpoint checkpoints/v21_newton_active_cable/score_based.ckpt \\
        --num-envs 1024 \\
        --batch-size 4096

추론 명령어:
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/inference_agent.py \\
        --checkpoint results/mimic_isaaclab_active_cable/score_based.ckpt \\
        --motion-file data/motion_for_trackers/skeleton_torque_suit_walk_constspeed_reduced_humanoid.pt \\
        --simulator isaaclab \\
        --num-envs 1 \\
        --cycle-seconds 60 \\
        --overrides "robot.asset.usd_asset_file_name=usd/skeleton_torque_suit/skeleton_torque_suit.usda"
"""
import argparse
...
def agent_config(
    ...
    # v21과 동일한 네트워크 구조 유지 (가중치 warm_start 호환)
