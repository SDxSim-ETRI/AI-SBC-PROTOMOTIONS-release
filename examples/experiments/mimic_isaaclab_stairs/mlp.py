...
#
"""
IsaacLab Stair Climbing Experiment
====================================

IsaacLab 파인튜닝 완료 체크포인트(v6/v8)에서 warm_start하여 계단 지형 적응 학습.
계단 전용 모션 불필요 — 기존 평지 모션 + 지형 높이 보정으로 계단 학습 가능.

지형 구성: flat 50% / stairs_up 25% / stairs_down 25%
계단 높이: 5cm (난이도 0) ~ 22.5cm (난이도 9), 커리큘럼 자동 조정

--- skeleton 학습 명령어 ---
    rm -rf results/mimic_isaaclab_stairs/
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/train_agent.py \\
        --robot-name skeleton_torque \\
        --simulator isaaclab \\
        --experiment-path examples/experiments/mimic_isaaclab_stairs/mlp.py \\
        --experiment-name mimic_isaaclab_stairs \\
        --motion-file data/motion_for_trackers/skeleton_torque_motions_11.pt \\
        --num-envs 1024 \\
        --batch-size 4096 \\
        --checkpoint checkpoints/v6_isaaclab_finetune/score_based.ckpt \\
        --training-max-steps 50000000

--- suit 학습 명령어 ---
    rm -rf results/mimic_isaaclab_stairs_suit/
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/train_agent.py \\
        ...
        --num-envs 1024 \\
        --batch-size 4096 \\
        --checkpoint checkpoints/v8_isaaclab_suit_finetune/score_based.ckpt \\
        --training-max-steps 50000000

--- 추론 명령어 (skeleton, mesh) ---
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/inference_agent.py \\
        --checkpoint results/mimic_isaaclab_stairs/score_based.ckpt \\
        --motion-file data/motion_for_trackers/skeleton_torque_motions_11.pt \\
        --simulator isaaclab \\
        --num-envs 1 \\
        --cycle-seconds 20 \\
        --overrides "robot.asset.usd_asset_file_name=usd/skeleton_torque_mesh/skeleton_torque_mesh.usda"
"""
