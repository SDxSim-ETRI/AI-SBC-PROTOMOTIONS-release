...
#
"""
IsaacLab Fine-Tuning Experiment
================================

Newton으로 사전학습된 skeleton_torque 정책을 IsaacLab(PhysX)에서 파인튜닝.

학습 명령어:
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/train_agent.py \\
        --robot-name skeleton_torque \\
        --simulator isaaclab \\
        --experiment-path examples/experiments/mimic_isaaclab_finetune/mlp.py \\
        --experiment-name mimic_isaaclab_finetune \\
...
추론 명령어 (파인튜닝 완료 후):
    /home/user/miniforge3/envs/env_isaaclab/bin/python protomotions/inference_agent.py \\
        --checkpoint results/mimic_isaaclab_finetune/score_based.ckpt \\
        --motion-file data/motion_for_trackers/skeleton_torque_motions_11.pt \\
        --simulator isaaclab \\
        --num-envs 1 \\
        --cycle-seconds 20
...
def agent_config(
    ...
    # 네트워크 구조는 Newton 체크포인트와 동일하게 유지 (가중치 로드 호환성)
    ...
    # 파인튜닝: learning rate를 원본(actor 2e-5, critic 1e-4)의 1/10로 낮춤
